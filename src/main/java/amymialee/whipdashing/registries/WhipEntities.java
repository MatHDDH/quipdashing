...
public class WhipEntities {
    ...
    public static final EntityType<LatchEntity> LATCH_ENTITY = registerEntity("latch_entity", FabricEntityTypeBuilder.<LatchEntity>create(SpawnGroup.MISC, LatchEntity::new).dimensions(EntityDimensions.fixed(2.0F, 2.0F)).trackRangeChunks(16).trackedUpdateRate(Integer.MAX_VALUE).build());

    public static void init() {}

    private static <T extends Entity> EntityType<T> registerEntity(String name, EntityType<T> entity) {
        return Registry.register(Registry.ENTITY_TYPE, Whipdashing.id(name), entity);
    }
}