...
import mathd.whipdashing.util.DashingProjectileWrapper;
...
public class HookEntity extends ProjectileEntity {
    ...
    public void onTrackedDataSet(TrackedData<?> data) {
        if (HOOK_ENTITY_ID.equals(data)) {
            ...
            this.setHookedEntity(i > 0 ? this.getWorld().getEntityById(i - 1) : null);
        }
    ...
    public void tick() {
        ...
        if (player == null) {
            this.remove(false);
        } else if (this.keepIfValid(player) || this.getWorld().isClient) {
            this.updateRotation();
            switch (this.getState()) {
                case FLYING -> {
                    this.checkForCollision();
                    this.move(MovementType.SELF, this.getVelocity());
                    if (this.groundCollision || this.horizontalCollision) {
                ...
                case PUSHING_PROJECTILE -> {
                    this.move(MovementType.SELF, this.getVelocity());
                    if (this.getHookedEntity() != null && !this.getHookedEntity().isRemoved() && this.getHookedEntity().getWorld().getRegistryKey() == this.getWorld().getRegistryKey()) {
                ...
                case RETURNING_EMPTY -> {
                    ...
                    if (this.getWorld().isClient() && this.distanceTo(player) < 0.1) {
                        this.discard();
                    }
                }
    ...
    public void setState(State state) {
        ...
        if (user != null) {
            if (state == State.RETURNING_EMPTY || state == State.RETURNING_PULLING || state == State.RETURNING_REFLECTING || state == State.PULLING_OWNER) {
                ...
                getWorld().playSound(null, user.getX(), user.getY(), user.getZ(), Whipdashing.HOOK_RETRIEVE, SoundCategory.PLAYERS, 4.0F, 1.8F);
            } else if (state == State.HOOKED_LIGHT_ENTITY || state == State.HOOKED_HEAVY_ENTITY) {
                getWorld().playSound(null, user.getX(), user.getY(), user.getZ(), Whipdashing.HOOK_RETRIEVE, SoundCategory.PLAYERS, 4.0F, 0.4F);
            }
        }
        this.state = state;
    }
    ...
    private void remove(boolean sound) {
        if (!getWorld().isClient()) {
            if (sound) {
                getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), Whipdashing.HOOK_RETURN, SoundCategory.PLAYERS, 0.6f, 4f);
            }
            this.discard();
        }
    }
    ...
    public void handleStatus(byte status) {
        if (status == 121 && this.getWorld().isClient) {
            this.setState(State.RETURNING_EMPTY);
        }
        if (status == 122 && this.getWorld().isClient) {
            this.setState(State.PULLING_OWNER);
        }
        Entity entity = this.getPlayerOwner();
        if (status == 123 && this.getWorld().isClient && entity != null && entity == MinecraftClient.getInstance().player) {
            this.setState(State.RETURNING_EMPTY);
    ...
    private void checkForCollision() {
        ...
        EntityHitResult entityHitResult = ProjectileUtil.getEntityCollision(getWorld(), this, pos, target, this.getBoundingBox().stretch(velocity).expand(2.0), this::canHit);
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (!this.getWorld().isClient && this.getHookedEntity() == null) {
            Entity entity = entityHitResult.getEntity();
            if ((entity instanceof LatchEntity latch && !latch.isActive()) || entity.getType().isIn(Whipdashing.IGNORED_ENTITIES) || entity.getCommandTags().contains("wd-ignored")) {
                return;
            }
    ...
    public void use() {
        ...
        if (!this.getWorld().isClient && playerEntity != null && this.keepIfValid(playerEntity)) {
    ...
    public boolean isHeavy(Entity entity, LivingEntity owner) {
        ...
        return entity.getType().isIn(Whipdashing.HEAVY_ENTITIES) || entity.getCommandTags().contains("wd-heavy") || entity.getType().getTrackTickInterval() == Integer.MAX_VALUE || ((this.getHookedEntity() instanceof LivingEntity living && living.getMaxHealth() > owner.getMaxHealth() * 1.5f));
    }
